...
class Signer
{
    ...
    public static function sign(
        ...
        if (!\NFePHP\Common\Validator::isXML($content)) {
            throw SignerException::isNotXml();
        }
        ...
        if (!self::existsSignature($content,$node)) {
    ...
    private static function createSignature(
        ...
        $signatureNode = $dom->createElement('ns:Signature');
        $root->appendChild($signatureNode);
        $signedInfoNode = $dom->createElement('ns:SignedInfo');
        $signatureNode->appendChild($signedInfoNode);
        $canonicalNode = $dom->createElement('ns:CanonicalizationMethod');
        $signedInfoNode->appendChild($canonicalNode);
        $canonicalNode->setAttribute('Algorithm', $nsCannonMethod);
        $signatureMethodNode = $dom->createElement('ns:SignatureMethod');
        $signedInfoNode->appendChild($signatureMethodNode);
        $signatureMethodNode->setAttribute('Algorithm', $nsSignatureMethod);
        $referenceNode = $dom->createElement('ns:Reference');
        $signedInfoNode->appendChild($referenceNode);
        if (!empty($idSigned)) {
            $idSigned = "#$idSigned";
        }
        $referenceNode->setAttribute('URI', $idSigned);
        $transformsNode = $dom->createElement('ns:Transforms');
        $referenceNode->appendChild($transformsNode);
        $transfNode1 = $dom->createElement('ns:Transform');
        $transformsNode->appendChild($transfNode1);
        $transfNode1->setAttribute('Algorithm', $nsTransformMethod1);
        $transfNode2 = $dom->createElement('ns:Transform');
        $transformsNode->appendChild($transfNode2);
        $transfNode2->setAttribute('Algorithm', $nsTransformMethod2);
        $digestMethodNode = $dom->createElement('ns:DigestMethod');
        $referenceNode->appendChild($digestMethodNode);
        $digestMethodNode->setAttribute('Algorithm', $nsDigestMethod);
        $digestValueNode = $dom->createElement('ns:DigestValue', $digestValue);
        $referenceNode->appendChild($digestValueNode);
        $c14n = self::canonize($signedInfoNode, $canonical);
        $signature = $certificate->sign($c14n, $algorithm);
        $signatureValue = base64_encode($signature);
        $signatureValueNode = $dom->createElement('ns:SignatureValue', $signatureValue);
        $signatureNode->appendChild($signatureValueNode);
        $keyInfoNode = $dom->createElement('ns:KeyInfo');
        $signatureNode->appendChild($keyInfoNode);
        $x509DataNode = $dom->createElement('ns:X509Data');
        $keyInfoNode->appendChild($x509DataNode);
        $pubKeyClean = $certificate->publicKey->unFormated();
        $x509CertificateNode = $dom->createElement('ns:X509Certificate', $pubKeyClean);
    ...
    public static function removeSignature($content)
    {
        ...
        $signature = $node->getElementsByTagName('ns:Signature')->item(0);
        if (!empty($signature)) {
     ...
     * @return boolean
     */
    public static function existsSignature($content, $node=null)
    {
        if (!\NFePHP\Common\Validator::isXML($content)) {
            throw SignerException::isNotXml();
        }
        ...
        $dom->preserveWhiteSpace = false;
        $dom->loadXML($content);
        if($node->firstChild->nodeName=="ns:Signature"){
          dd($node);
          $signature = $node->firstChild;
        }
        elseif($node->lastChild->nodeName=="ns:Signature"){
          $signature = $node->lastChild;
        }
        else{
          $signature=null;
